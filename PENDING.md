...
watsonx, see Phase 16). Blocked on: the `[vertex]` extra actually being in
the deployed image, `REPOGUARD_AI_PROVIDER=vertex` set on the service, and
`roles/aiplatform.user` granted to the Cloud Run runtime service account —
first two fixed on `fix/cd-cors-vercel-origin` (not yet merged), the IAM
grant needs a human to run it (`gcloud projects add-iam-policy-binding`;
blocked here by Claude Code's own permission-grant restriction, not a
project decision). Confirmed live before the fix: `ok=false`,
